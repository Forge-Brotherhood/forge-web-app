...
import { usePrayerListQuery } from "@features/prayer";
import type { APIPrayerListItem } from "@core/models/apiModels";
...
export function usePrayerExperience() {
  ...
  useEffect(() => {
    ...
    if (prayerListData.items && prayerListData.items.length > 0) {
      ...
      prayers.forEach((item: APIPrayerListItem, index: number) => {
        const thread = item.thread;
        const authorName = thread?.isAnonymous
          ? "Someone"
          : thread?.author?.firstName || "Someone";
        ...
        finalStages.push({
          ...
          content: {
            ...
            prayerText: thread?.posts?.[0]?.content || "",
            authorName,
